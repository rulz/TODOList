...
class TaskApiTest(TestCaseUtils, TestCase):
    ...
    def setUp(self):
        ...
        self.task2 = Task.objects.create(
            name='tarea', description='description tarea', owner=self.user2,
        ...
        self.response = self.client.post(
            reverse('oauth2_provider:token'), data=self.token_request_data, 
            **self.auth_headers
        )
    ...
    def test_create_task(self):
        ...
        response = self.client.post(
            reverse('tasks-list'), data=json.dumps(data), 
    ...
    def test_list_task_three(self):
        task = Task.objects.create(
            name='tarea2', description='description tarea2', 
            done=True, owner=self.user
        )
        response = self.client.get(reverse('tasks-list'), **self.headers)
        content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_detail_task(self):
        response = self.client.get(
            reverse('tasks-detail', kwargs={'pk': 1}), **self.headers
        )
        content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_task(self):
        ...
        response = self.client.put(
            reverse('tasks-detail', kwargs={'pk': 1}), 
    ...
    def test_assigned_to_task(self):
        ...
        response = self.client.put(
            reverse('tasks-detail', kwargs={'pk': 1}), data=json.dumps(data),
            content_type='application/json', **self.headers
        )
        content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 200)
...
class UserApiTest(TestCaseUtils, TestCase):
    ...
    def test_update_user(self):
        ...
        response = self.client.put(
            reverse('users-detail', kwargs={'pk': 1}), data=json.dumps(data),
            content_type='application/json', **self.headers
        )
        content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_user(self):
        data = {'username': 'rulz_new',
                'email': 'rulznew@gmail.com', 'password': '1234'}
        response = self.client.post(
            reverse('users-list'), data=json.dumps(data),
            content_type='application/json', **self.headers
        )
        content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 201)
